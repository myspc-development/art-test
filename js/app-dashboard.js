...
var APDashboardApp = (function (React$1, require$$0, Chart, GridLayout) {
  ...
  // Ensure the react-grid-layout library is available.
  if (typeof GridLayout === 'undefined' && window.ReactGridLayout) {
    GridLayout = window.ReactGridLayout;
  }
  if (GridLayout && GridLayout.WidthProvider && GridLayout.Responsive) {
    // Wrap Responsive grid with WidthProvider for responsive layouts
    GridLayout = GridLayout.WidthProvider(GridLayout.Responsive);
  } else {
    console.error('react-grid-layout failed to load');
    return;
  }
