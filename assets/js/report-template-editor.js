jQuery(function ($) {
  ...
  function load(type, cb) {
    fetch(apiRoot + 'artpulse/v1/report-template/' + type, {
      headers: { 'X-WP-Nonce': nonce }
    }).then(r => r.json()).then(cb);
  }

  function save(type, data) {
    return fetch(apiRoot + 'artpulse/v1/report-template/' + type, {
      method: 'POST',
  ...
  function renderBudget(tpl) {
    ...
    function update() {
      var sum = 0;
      tbody.find('.est').each(function(){ sum += parseFloat(this.value)||0; });
    ...
    saveBtn.on('click', function(){
      ...
      tbody.find('tr').each(function(){
        ...
        if (item || est || act) rows.push({item:item, estimated:est, actual:act});
      });
  ...
  function renderImpact(tpl) {
    ...
    saveBtn.on('click', function(){
      ...
      tbody.find('tr').each(function(){
        ...
        if (m || v) rows.push({metric:m, value:v});
      });
