	...
	<div class="ap-event-info">
		...
		<?php if ( $start ) : ?>
			<p class="ap-event-date">
				...
				if ( $end ) :
					?>
					– <?php echo esc_html( date_i18n( 'M j, Y g:i a', strtotime( $end ) ) ); ?><?php endif; ?>
			</p>
		<?php endif; ?>
		...
		<?php if ( $org_name || $org_email ) : ?>
			<span class="screen-reader-text"><?php esc_html_e( 'Organizer:', 'artpulse' ); ?> <?php echo esc_html( $org_name ); ?>
			<?php
			if ( $org_email ) :
				?>
				(<?php echo \ArtPulse\Util\ap_obfuscate_email( $org_email ); ?>)<?php endif; ?></span>
		<?php endif; ?>
		<?php
