...
<article id="post-<?php the_ID(); ?>" <?php post_class('ap-artist-profile'); ?>>
  ...
  <ul class="ap-artist-social">
    ...
    <?php if ($instagram): ?><li><a href="<?php echo esc_url($instagram); ?>">Instagram</a></li><?php endif; ?>
    <?php if ($website): ?><li><a href="<?php echo esc_url($website); ?>">Website</a></li><?php endif; ?>
  </ul>
  <?php if ($images): ?>
...
<?php
    ...
    $graph[] = [
      ...
      'url'  => get_permalink(),
      'description' => $bio,
      'sameAs' => array_values(array_filter([$twitter, $instagram, $website])),
    ];
    if ($images) {
    ...
    echo '<script type="application/ld+json">' . wp_json_encode(['@context' => 'https://schema.org', '@graph' => $graph]) . '</script>';
  endwhile;
endif;
get_footer();
