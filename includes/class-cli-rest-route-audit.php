...
class AP_CLI_Rest_Route_Audit
{
    ...
    public function __invoke(array $args, array $assoc_args): void
    {
        ...
        if (isset($assoc_args['json'])) {
            \WP_CLI::print_value(array_values($conflicts), ['json' => true]);
            return;
        }
        ...
        foreach ($conflicts as $conflict) {
            \WP_CLI::log(sprintf('%s %s', $conflict['method'], $conflict['path']));
            foreach ($conflict['callbacks'] as $cb) {
                $line = sprintf('  - %s', $cb['description']);
                if ($cb['location']) {
                    $line .= sprintf(' (%s)', $cb['location']);
                }
                if ($cb['plugin']) {
                    $line .= sprintf(' [%s]', $cb['plugin']);
                }
                \WP_CLI::log($line);
            }
    ...
    public function find_conflicts(): array
    {
        ...
        foreach ($routes as $path => $endpoints) {
            foreach ($endpoints as $endpoint) {
                $methods = $this->normalize_methods($endpoint['methods'] ?? []);
        ...
        foreach ($map as $key => $data) {
            $unique = array_unique(array_map(static function ($cb) {
    ...
    {
        return [
            sprintf("Wrap custom register_rest_route() in: if ( ! rest_route_exists( '%s' ) ) { register_rest_route( ... ); }", $path),
