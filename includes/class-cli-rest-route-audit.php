...
class AP_CLI_REST_Route_Audit {
    ...
    public function __invoke(): void {
        ...
        foreach ($routes as $route => $endpoints) {
            ...
            foreach ($endpoints as $endpoint) {
                ...
                $signature = $this->callback_signature($callback);
                $file      = $this->callback_file($callback);
                $plugin    = $this->callback_plugin($file);
     ...
     */
    private function callback_signature($callback): string {
     ...
     */
    private function callback_file($callback): ?string {
        try {
     ...
     */
    private function callback_plugin(?string $file): ?string {
        if (!$file) {
            return null;
        }
...
if (defined('WP_CLI') && WP_CLI) {
    \WP_CLI::add_command('rest audit', 'AP_CLI_REST_Route_Audit');
}
