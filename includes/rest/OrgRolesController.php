...
class OrgRolesController extends WP_REST_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			$this->namespace,
			'/' . $this->rest_base . '/(?P<role_key>[a-zA-Z0-9_-]+)',
			array(
				array(
					'methods'             => 'PUT',
	...
	public function create_role( $request ) {
		...
		if ( ! $role_key || ! $label ) {
			return new \WP_Error( 'invalid_data', 'Invalid role data', array( 'status' => 400 ) );
		}

		$table  = $wpdb->prefix . 'ap_roles';
		$exists = $wpdb->get_var( $wpdb->prepare( "SELECT role_key FROM $table WHERE role_key = %s", $role_key ) );
		if ( $exists ) {
			return new \WP_Error( 'role_exists', 'Role already exists', array( 'status' => 409 ) );
		}
	...
	public function update_role( $request ) {
		...
		if ( empty( $data ) ) {
			return new \WP_Error( 'invalid_data', 'Nothing to update', array( 'status' => 400 ) );
		}

		$table = $wpdb->prefix . 'ap_roles';
		$wpdb->update( $table, $data, array( 'role_key' => $role_key ) );
	...
	public function delete_role( $request ) {
		...
		if ( in_array( $role_key, $protected, true ) ) {
			return new \WP_Error( 'protected_role', 'Cannot delete protected role', array( 'status' => 400 ) );
		}
		...
		if ( $in_use > 0 ) {
			return new \WP_Error( 'role_in_use', 'Role is assigned to users', array( 'status' => 400 ) );
		}

		$table = $wpdb->prefix . 'ap_roles';
		$wpdb->delete( $table, array( 'role_key' => $role_key ) );
