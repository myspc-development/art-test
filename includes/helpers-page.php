...
{
    $pages = [
        ['login', 'Login', '[ap_login]', 'page-login.php'],
        ['dashboard', 'Dashboard', '', 'page-dashboard.php'],
        ['events', 'Events', '', 'page-events.php'],
        ['artists', 'Artists', '', 'page-artists.php'],
    ];
...
function artpulse_template_loader(string $template): string
{
    ...
    if (is_page(array_keys($map))) {
        ...
        if (isset($map[$slug])) {
            $file = plugin_dir_path(dirname(__FILE__)) . 'templates/' . $map[$slug];
            if (file_exists($file)) {
                return $file;
            }
        }
