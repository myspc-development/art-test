...
if ( have_posts() ) :
	while ( have_posts() ) :
		...
		if ( $venue ) {
			echo '<li><strong>Venue:</strong> ' . esc_html( $venue ) . '</li>';
		}
		if ( $address ) {
		...
		if ( $contact ) {
			echo '<li><strong>Contact:</strong> ' . esc_html( $contact ) . '</li>';
		}
		if ( $rsvp ) {
			echo '<li><strong>RSVP:</strong> <a href="' . esc_url( $rsvp ) . '" target="_blank">Reserve</a></li>';
		}
		echo '</ul>';
