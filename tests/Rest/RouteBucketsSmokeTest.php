...
class RouteBucketsSmokeTest extends WP_UnitTestCase {
		...
		array(
			...
			'methods' => array(
				'POST' => array(
					...
					'payload'      => array(
						'widget_roles' => array( 'member' => array( 'widget_one' ) ),
						'role_widgets' => array( 'member' => array( 'widget_one' ) ),
						'locked'       => array( 'widget_one' ),
					),
		...
		array(
			...
			'methods' => array(
				'GET' => array(
					...
					'shape' => array(), // array of role slugs
				),
			),
		),
		array(
			...
			'methods' => array(
				'GET' => array(
					...
					'shape' => array(), // array of widget defs
				),
			),
		),
		array(
			...
			'methods' => array(
				'GET' => array(
					...
					'shape' => array(), // array/object settings
				),
			),
		),
		array(
			...
			'methods' => array(
				'POST' => array(
					...
					'shape'   => array( 'saved' ), // many controllers return { saved: true }
				),
			),
		),
		array(
	...
	public function set_up(): void {
		...
		$this->reader_id = self::factory()->user->create( array( 'role' => get_role( 'member' ) ? 'member' : 'subscriber' ) );
	...
	private function dispatch( string $method, string $path, ?array $payload, array $expect, bool $as_admin = false ): WP_REST_Response|WP_Error {
		...
		if ( in_array( $method, array( 'POST', 'PUT', 'PATCH' ), true ) && $payload !== null ) {
