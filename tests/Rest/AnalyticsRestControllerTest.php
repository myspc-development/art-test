...
class AnalyticsRestControllerTest extends \WP_UnitTestCase
{
    ...
    public function test_trends_and_export(): void
    {
        ...
        $wpdb->insert($metric_table, ['event_id'=>$this->event_id,'metric'=>'view','day'=>$today,'count'=>5]);
        $wpdb->insert($metric_table, ['event_id'=>$this->event_id,'metric'=>'favorite','day'=>$today,'count'=>2]);
        $wpdb->insert($metric_table, ['event_id'=>$this->event_id,'metric'=>'view','day'=>$yesterday,'count'=>3]);

        $wpdb->insert($this->ticket_table, [
            'user_id' => 1,
        ...
        $this->assertSame([3,5], $data['views']);
