...
class QuickAddEventWidgetTest extends TestCase {
    protected function setUp(): void {
        ...
        when('ArtPulse\Core\get_option')->justReturn([]);
        when('ArtPulse\Core\get_userdata')->justReturn((object)['roles' => ['organization']]);
        when('sanitize_key')->alias(fn($v) => $v);
        when('apply_filters')->alias(function($tag, $value){ return $value; });
    }
