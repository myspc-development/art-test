...
class EventSubmissionShortcodeTest extends \WP_UnitTestCase {
	...
	public function test_namespaced_redirect_stubs_used_when_globals_disabled(): void {
			...
			$refHandle   = \Patchwork\redefine(
				'\\wp_get_referer',
				function () use ( &$globalRefCalled ) {
					$globalRefCalled = true;
					return '/global';
				}
			);
			$redirHandle = \Patchwork\redefine(
				'\\wp_safe_redirect',
				function ( $url ) use ( &$globalRedirectCalled ) {
	...
	public function test_meta_logged_when_banner_upload_fails(): void {
			// Banner upload fails while an additional image succeeds.
			\ArtPulse\Frontend\StubState::$media_returns = array(
				'event_banner' => new \WP_Error( 'upload_error', 'bad' ),
				'ap_image'     => 11,
			);
			$_FILES['event_banner']                      = array(
				'name'     => 'b.jpg',
			...
			try {
					...
					$this->assertSame( 'Error uploading banner. Please try again.', \ArtPulse\Frontend\StubState::$notice );
			}
	}
