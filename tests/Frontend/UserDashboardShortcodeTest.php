...
class UserDashboardShortcodeTest extends WP_UnitTestCase {
	...
	public function provide_roles(): array {
		return array(
			'subscriber'    => array( 'subscriber', 'member' ),
			'contributor'   => array( 'contributor', 'member' ),
			'author'        => array( 'author', 'member' ),
			'editor'        => array( 'editor', 'member' ),
			'member'        => array( 'member', 'member' ),
			'artist'        => array( 'artist', 'artist' ),
			'organization'  => array( 'organization', 'organization' ),
			'administrator' => array( 'administrator', 'organization' ),
		);
	}
	...
	public function test_user_dashboard_matches_render_for_role( string $wpRole, string $queryRole ): void {
		...
		$uid = self::factory()->user->create( array( 'role' => $wpRole ) );
