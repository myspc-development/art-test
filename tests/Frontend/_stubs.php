...
$GLOBALS['ap_testing_user_meta']    = array(); // [user_id => [meta_key => value]]
$GLOBALS['__ap_test_user_meta']     =& $GLOBALS['ap_testing_user_meta']; // back-compat

function ap_set_current_user_role( string $role ): void {
	$GLOBALS['ap_testing_current_user']['roles'] = array( $role );
	// update MockStorage if available
	if ( class_exists( 'ArtPulse\\Tests\\Stubs\\MockStorage' ) ) {
		\ArtPulse\Tests\Stubs\MockStorage::$users[ $GLOBALS['ap_testing_current_user']['ID'] ] = new \WP_User( $GLOBALS['ap_testing_current_user']['ID'], array( $role ) );
	}
}
function ap_set_user_meta( int $user_id, string $key, $value ): void {
	if ( ! isset( $GLOBALS['ap_testing_user_meta'][ $user_id ] ) ) {
		$GLOBALS['ap_testing_user_meta'][ $user_id ] = array();
	}
...
function get_current_user_id(): int {
	return $GLOBALS['ap_testing_current_user']['ID']; }
...
function user_can( $user, string $cap ): bool {
	...
	$roles = is_object( $user ) ? ( $user->roles ?? array() ) : ( $GLOBALS['ap_testing_current_user']['roles'] ?? array() );
	...
	if ( $cap === 'edit_posts' ) {
		return in_array( 'editor', $roles, true ) || in_array( 'author', $roles, true );
	}
	return false;
}
function current_user_can( string $cap ): bool {
	return user_can( null, $cap ); }
...
function get_user_meta( int $user_id, string $key, bool $single = false ) {
	...
	return $store === null ? array() : (array) $store;
}
