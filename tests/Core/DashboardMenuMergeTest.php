...
namespace {
    ...
    if (!class_exists('WP_User')) { class WP_User { public $roles = []; public function __construct($id = 0) {} } }
    if (!function_exists('apply_filters')) { function apply_filters($tag, $value) { return $value; } }
    if (!function_exists('add_filter')) { function add_filter($tag, $func, $priority = 10, $args = 1) {} }
...
namespace ArtPulse\Core\Tests {
    ...
    class DashboardMenuMergeTest extends TestCase {
        ...
        public function test_org_menu_includes_transactions() {
            $menu = \ap_merge_dashboard_menus(['organization'], true);
            $ids = array_column($menu, 'id');
