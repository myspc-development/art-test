...
final class DashboardPreviewRoleHeaderTest extends TestCase
{
    protected function setUp(): void
    {
        ...
        MockStorage::$options = ['ap_dashboard_option' => 'value'];
    }
    ...
    public function test_preview_role_sets_header_and_attributes_without_persisting(): void
    {
        ...
        $roleHandle = \Patchwork\redefine('ap_get_effective_role', function () {
            return \ArtPulse\Core\RoleResolver::resolve();
        });

        $captured = [];
        $headerHandle = \Patchwork\redefine('header', function ($string) use (&$captured) {
            $captured[] = $string;
        });
        ...
        $this->assertArrayNotHasKey('ap_dashboard_layout', MockStorage::$user_meta[1] ?? []);
