...
function main() {
  ...
  for (const [id, cfg] of Object.entries(phpWidgets)) {
    ...
    if (label && /legacy|deprecated/i.test(label)) continue;
    ...
    if (!arraysEqual(jsonEntry.allowed_roles || [], roles)) {
      ...
      jsonChanged = true;
      console.log(`Updated roles for ${id} in available-widgets.json`);
    }
    if (label) jsonEntry.title = label;
    if (cfg.description) jsonEntry.description = cfg.description;
    if (cfg.category) jsonEntry.category = cfg.category;

    const reactFile = findReactFile(jsonEntry);
    if (!reactFile || !roles.length) continue;
