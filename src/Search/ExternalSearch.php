...
class ExternalSearch {
    ...
    public static function is_enabled(): bool {
        ...
        return in_array($service, ['algolia', 'elasticpress'], true);
    }
    ...
    public static function search(string $type, array $args): array {
        $service = self::get_service();
        if ($service === 'algolia') {
            $results = apply_filters('algolia_search_records', [], $type, $args);
            return is_array($results) ? $results : [];
        }
        if ($service === 'elasticpress' && function_exists('ep_search')) {
            $query = ep_search(array_merge(['post_type' => 'artpulse_' . $type], $args));
            return $query instanceof WP_Query ? $query->posts : [];
        }
        return [];
    }
}
