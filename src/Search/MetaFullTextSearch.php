...
class MetaFullTextSearch {
	public static function register() {
		add_filter( 'posts_search', array( self::class, 'posts_search_meta' ), 10, 2 );

		// REST API filters for meta search on all CPTs
		foreach ( array( 'artpulse_artist', 'artpulse_artwork', 'artpulse_event', 'artpulse_org' ) as $post_type ) {
			add_filter(
	...
	public static function posts_search_meta( $search, $query ) {
		...
		if ( ! in_array( $post_type, array( 'artpulse_artist', 'artpulse_artwork', 'artpulse_event', 'artpulse_org' ), true ) ) {
			return $search;
		}
