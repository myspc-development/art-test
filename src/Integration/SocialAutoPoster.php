...
class SocialAutoPoster
{
    ...
    private const NETWORKS   = ['facebook', 'instagram', 'twitter', 'pinterest'];
    ...
    public static function register(): void
    {
        ...
        add_action('init', [self::class, 'register_publish_hooks']);
    }
    ...
    public static function register_publish_hooks(): void
    {
        ...
        foreach (self::POST_TYPES as $key => $post_type) {
            if (!empty($opts['post_types'][$key])) {
                add_action("publish_{$post_type}", [self::class, 'handle_publish'], 10, 2);
            }
        }
    }
    ...
    private static function post_facebook(string $msg, array $cfg): void
    {
        ...
        wp_remote_post($url, [
            'body'    => [ 'message' => $msg, 'access_token' => $cfg['token'] ],
            'timeout' => 5,
        ]);
    }
    ...
    private static function post_instagram(string $msg, array $cfg): void
    {
        ...
        wp_remote_post($url, [
            'body'    => [ 'caption' => $msg, 'access_token' => $cfg['token'] ],
            'timeout' => 5,
        ]);
    }
