...
class RecommendationPreferenceManager {
	...
	public static function get( int $user_id ): array {
		...
		return array(
			'preferred_tags' => $row ? json_decode( $row['preferred_tags'], true ) ?: array() : array(),
			'ignored_tags'   => $row ? json_decode( $row['ignored_tags'], true ) ?: array() : array(),
			'blacklist_ids'  => $row ? json_decode( $row['blacklist_ids'], true ) ?: array() : array(),
		);
	}
