...
class ArtistDashboardShortcode {
	public static function register(): void {
		...
		add_action( 'wp_ajax_ap_delete_artwork', array( self::class, 'handle_ajax_delete_artwork' ) );
		add_action( 'wp_ajax_save_artwork_order', array( self::class, 'handle_save_artwork_order' ) );
	...
	public static function handle_ajax_delete_artwork(): void {
		...
		if ( ! $post || $post->post_type !== 'artpulse_artwork' || $post->post_author != get_current_user_id() ) {
			wp_send_json_error( array( 'message' => 'Invalid artwork.' ) );
		}
