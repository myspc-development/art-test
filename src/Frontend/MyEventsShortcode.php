...
class MyEventsShortcode {
	...
	public static function register() {
		\ArtPulse\Core\ShortcodeRegistry::register( 'ap_my_events', 'My Events', array( self::class, 'render' ) );
		add_action( 'init', array( self::class, 'handle_deletion' ) );
		add_action( 'init', array( self::class, 'handle_bulk_actions' ) );
	...
	public static function render( $atts ) {
		...
		wp_enqueue_script( 'ap-dashboard-calendar', plugin_dir_url( ARTPULSE_PLUGIN_FILE ) . 'assets/js/ap-dashboard-calendar.js', array( 'fullcalendar-js' ), '1.0', true );
		wp_localize_script(
			'ap-dashboard-calendar',
			'APArtistCalendar',
			array(
	...
	public static function handle_deletion() {
		...
		if ( $event && $event->post_type === 'artpulse_event' && $event->post_author == get_current_user_id() ) {
			...
			wp_safe_redirect( remove_query_arg( array( 'ap_delete_event', 'ap_nonce' ) ) );
			exit;
		}
	}
			...
			switch ( $action ) {
				...
				case 'duplicate':
					...
					if ( ! is_wp_error( $new_id ) ) {
						$meta_keys = array( '_ap_event_date', 'event_start_date', 'event_end_date', 'event_recurrence_rule' );
