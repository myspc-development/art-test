...
class ArtistProfileFormShortcode {
	public static function register(): void {
		\ArtPulse\Core\ShortcodeRegistry::register( 'ap_artist_profile_form', 'Artist Profile Form', array( self::class, 'render_form' ) );
		add_action( 'wp_enqueue_scripts', array( self::class, 'enqueue_styles' ) );
		add_action( 'init', array( self::class, 'maybe_handle_form' ) );
	}
	...
	public static function render_form(): string {
		...
		<form method="post" enctype="multipart/form-data" class="ap-artist-profile-form ap-form-container bg-white rounded-2xl shadow p-6 space-y-4" data-no-ajax="true">
			...
			<input type="hidden" name="address_components" id="ap-address-components" value="
			<?php
			echo esc_attr(
				json_encode(
					array(
