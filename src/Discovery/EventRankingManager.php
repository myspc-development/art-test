...
class EventRankingManager
{
    public static function register(): void
    {
        ...
        add_action('ap_event_ranking_calculate', [self::class, 'calculate_scores']);
        add_filter('the_content', [self::class, 'maybe_debug_score']);
    }
    ...
    public static function maybe_debug_score(string $content): string
    {
        if (is_singular('artpulse_event') && isset($_GET['show_score']) && $_GET['show_score'] == '1') {
            ...
            return '<p>' . sprintf(esc_html__('Score: %s', 'artpulse'), esc_html($score)) . '</p>' . $content;
        }
        return $content;
    }
}
