...
class OpenAIClient {
	...
	private static function request( array $messages, int $max_tokens ) {
		...
		if ( ! $key ) {
			return new WP_Error( 'missing_api_key', __( 'OpenAI API key not configured.', 'artpulse' ), array( 'status' => 500 ) );
		}
		...
		if ( is_wp_error( $response ) ) {
			...
			return new WP_Error( 'openai_request_failed', __( 'Failed to contact OpenAI.', 'artpulse' ), array( 'status' => 500 ) );
		}

		$code = wp_remote_retrieve_response_code( $response );
		if ( $code >= 429 ) {
			self::log_error( 'Rate limit or server error: ' . $code );
			return new WP_Error( 'openai_rate_limited', __( 'OpenAI rate limit reached.', 'artpulse' ), array( 'status' => 429 ) );
		}
		if ( $code < 200 || $code >= 300 ) {
			self::log_error( 'HTTP error: ' . $code );
			return new WP_Error( 'openai_http_error', __( 'OpenAI API error.', 'artpulse' ), array( 'status' => $code ) );
		}

		$data = json_decode( wp_remote_retrieve_body( $response ), true );
		if ( ! isset( $data['choices'][0]['message']['content'] ) ) {
			self::log_error( 'Invalid response: ' . wp_remote_retrieve_body( $response ) );
			return new WP_Error( 'openai_invalid_response', __( 'Invalid OpenAI response.', 'artpulse' ), array( 'status' => 500 ) );
		}
