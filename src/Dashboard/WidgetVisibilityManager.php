...
class WidgetVisibilityManager {
	 ...
	 */
	private static array $hidden_widgets = array();
	...
	public static function register(): void {
		add_action( 'wp_dashboard_setup', array( self::class, 'filter_visible_widgets' ), 99 );
		add_action( 'admin_notices', array( self::class, 'render_admin_notices' ) );
		add_action( 'admin_notices', array( self::class, 'render_empty_state_notice' ), 100 );
	}
	...
	public static function get_visibility_rules(): array {
		...
		$rules += array(
			'dashboard_activity'        => array( 'capability' => 'edit_posts' ),
			'dashboard_quick_press'     => array( 'capability' => 'edit_posts' ),
			'dashboard_site_health'     => array( 'capability' => 'view_site_health_checks' ),
			'artpulse_analytics_widget' => array( 'capability' => 'view_analytics' ),
		);
		...
		foreach ( $saved as $widget => $config ) {
			...
			if ( $merged ) {
				$rules[ $widget ] = array_merge( $rules[ $widget ] ?? array(), $merged );
			}
		}
	...
	public static function isVisible( string $widget, ?int $user_id = null, ?string $force_role = null ): bool {
		...
		$previewing      = $preview_enabled && in_array( $force_role, array( 'member', 'artist', 'organization' ), true );
		...
		$roles = $previewing
			? array( $force_role )
			: self::normalizeRoleList( $user->roles ?? array() );

		$role_for_hidden = $previewing ? $force_role : ( $roles[0] ?? '' );
		$hidden          = get_option( 'artpulse_dashboard_hidden_' . $role_for_hidden, array() );
		if ( is_string( $hidden ) ) {
			$decoded = json_decode( $hidden, true );
			$hidden  = is_array( $decoded ) ? $decoded : array();
		}
		...
		$config = $rules[ $widget ] ?? array();

		$cap     = $config['capability'] ?? null;
		$exclude = self::normalizeRoleList( $config['exclude_roles'] ?? array() );
		$allowed = self::normalizeRoleList( $config['allowed_roles'] ?? array() );
	...
	public static function filter_visible_widgets( $user = null ): void {
		...
		$roles = $preview_valid
			? array( $preview )
			: self::normalizeRoleList( $current_user->roles ?? array() );
		...
		foreach ( $rules as $widget => $config ) {
			$cap          = $config['capability'] ?? null;
			$exclude_raw  = $config['exclude_roles'] ?? array();
			$allowed      = self::normalizeRoleList( $config['allowed_roles'] ?? array() );
