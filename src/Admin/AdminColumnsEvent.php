...
class AdminColumnsEvent {
	...
	public static function register() {
		add_filter( 'manage_artpulse_event_posts_columns', array( __CLASS__, 'add_columns' ) );
		add_action( 'manage_artpulse_event_posts_custom_column', array( __CLASS__, 'render_columns' ), 10, 2 );
		add_filter( 'manage_edit-artpulse_event_sortable_columns', array( __CLASS__, 'make_sortable' ) );
		add_action( 'wp_ajax_ap_save_event_gallery_order', array( __CLASS__, 'ajax_save_gallery_order' ) );
	}
			...
			case 'event_gallery':
				...
				if ( is_array( $ids ) && $ids ) {
					...
					foreach ( $ids as $img_id ) {
						...
						echo wp_get_attachment_image( $img_id, array( 40, 40 ) );
						if ( $link ) {
							echo '</a>';
						}
						echo '</li>';
