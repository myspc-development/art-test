...
class ReportingManager {
	...
	public static function register_routes(): void {
		if ( ! ap_rest_route_registered( ARTPULSE_API_NAMESPACE, '/admin/export' ) ) {
			register_rest_route(
				ARTPULSE_API_NAMESPACE,
				'/admin/export',
				array(
					'methods'             => 'GET',
					'callback'            => array( self::class, 'handle_export' ),
	...
	public static function handle_export( \WP_REST_Request $request ) {
		...
		if ( $type !== 'attendance' ) {
			return new \WP_Error( 'invalid_type', 'Unsupported export type.', array( 'status' => 400 ) );
		}
		if ( ! $event_id ) {
		...
		fputcsv( $stream, array( 'ticket_id', 'user_id', 'code', 'status', 'purchase_date' ) );
		foreach ( $rows as $row ) {
			fputcsv( $stream, array( $row['id'], $row['user_id'], $row['code'], $row['status'], $row['purchase_date'] ) );
		}
		rewind( $stream );
