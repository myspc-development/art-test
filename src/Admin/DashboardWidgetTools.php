...
class DashboardWidgetTools {
	...
	public static function get_role_widgets_for_current_user(): array {
		...
		foreach ( $roles as $role ) {
			foreach ( $all[ $role ] ?? array() as $widget ) {
				$id = $widget['id'] ?? null;
	...
	public static function register_default_widgets_for_role( string $role ): void {
		$defaults = array(
			'member'       => array(
				array(
					'id'       => 'welcome_box',
					'label'    => 'Welcome',
					'callback' => array( WelcomeBoxWidget::class, 'render' ),
				),
			),
			'artist'       => array(
				array(
		...
		if ( $uid > 0 ) {
			foreach ( $defaults[ $role ] ?? array() as $widget ) {
	...
	public static function register(): void {
		...
		add_action(
			'artpulse_render_settings_tab_widgets',
			function () {
	...
	public static function render(): void {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( __( 'Access denied.', 'artpulse' ), '', array( 'response' => 403 ) );
		}
	...
	public static function render_widget_preview( string $id ): string {
		...
		if ( ! is_callable( $cb ) ) {
			$cb = array( DashboardWidgetRegistry::class, 'render_widget_fallback' );
		}
	...
	public static function listWidgetsForRole( string $role, bool $simulate = false, bool $allow_placeholders = false ): array {
		...
		$preview_html = $renderer->renderIds(
			$all_ids,
			array(
		...
		foreach ( $all_ids as $cid ) {
			$def            = $registry[ $cid ] ?? array();
			$snap           = $snapshot[ $cid ] ?? array();
			...
			$allowed_roles       = $role_map[ $cid ] ?? ( $def['roles'] ?? array() );
	...
	public static function render_user_dashboard( int $user_id ): void {
		...
		self::render_layout_widgets(
			...
			function ( array $def, string $id, bool $visible ) use ( $user_id ): void {
				...
				echo '<span class="widget-title">' . artpulse_dashicon( $icon, array( 'style' => 'margin-right:6px;' ) ) . esc_html( $label ) . '</span>';
				echo '</div>';
	...
	public static function render_role_dashboard_preview( string $role ): void {
		...
		if ( $style ) {
			...
			if ( ! empty( $style['padding'] ) ) {
				...
				if ( $pad === 's' ) {
					$pad = '4px'; } elseif ( $pad === 'm' ) {
					$pad = '8px'; } elseif ( $pad === 'l' ) {
						$pad = '16px'; }
