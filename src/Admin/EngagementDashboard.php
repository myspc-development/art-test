...
class EngagementDashboard {
	...
	public static function render() {
		...
		$all_users = get_users(
			array(
				'role__in' => array( 'subscriber', 'artpulse_member', 'artpulse_pro', 'artpulse_org' ),
				'orderby'  => 'registered',
				'order'    => 'DESC',
				'number'   => 9999,
			)
		...
		foreach ( $all_users as $user ) {
			...
			if ( $activity_filter === 'active' && ! $is_active ) {
				continue;
			}
			if ( $activity_filter === 'inactive' && $is_active ) {
				continue;
			}
		...
		if ( isset( $_GET['ap_export_csv'] ) ) {
			...
			fputcsv( $output, array( 'Name', 'Email', 'Logins', 'Artworks', 'Events', 'Followers', 'Following', 'Score' ) );
