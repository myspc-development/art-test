...
class MetaBoxesCollection {
	public static function register(): void {
		add_action( 'add_meta_boxes_ap_collection', array( self::class, 'add_box' ) );
		add_action( 'save_post_ap_collection', array( self::class, 'save_box' ), 10, 2 );
		add_action( 'admin_enqueue_scripts', array( self::class, 'enqueue_assets' ) );
		add_action( 'wp_ajax_ap_search_collection_items', array( self::class, 'ajax_search_items' ) );
	}
	...
	public static function enqueue_assets( string $hook ): void {
		...
		wp_enqueue_script(
			...
			array( 'jquery', 'ap-admin-relationship', 'jquery-ui-sortable' ),
			ARTPULSE_VERSION,
			true
		);
	}
	...
	public static function ajax_search_items(): void {
		...
		$args = array(
			'post_type'      => array( 'artpulse_artwork', 'artpulse_event', 'artpulse_artist' ),
			'post_status'    => 'publish',
			'posts_per_page' => 20,
			's'              => $term,
			'fields'         => 'ids',
