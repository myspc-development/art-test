...
class ScheduledMessageManager {
	...
	public static function register(): void {
		...
		add_action( 'ap_process_scheduled_messages', array( self::class, 'process_due_messages' ) );
		self::schedule_cron();
	}
	...
	public static function process_due_messages(): void {
		...
		foreach ( $rows as $row ) {
			...
			$wpdb->update( $table, array( 'status' => 'sent' ), array( 'id' => $row['id'] ) );
		}
	}
