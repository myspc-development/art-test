...
class QuickStartGuide {
	...
	public static function register() {
		...
		add_action( 'admin_notices', array( self::class, 'welcomeNotice' ) );
		add_action( 'admin_init', array( self::class, 'handleDismiss' ) );
	}
	...
	public static function welcomeNotice() {
		...
		echo '<p>' . sprintf( esc_html__( 'Welcome to ArtPulse! Visit the %1$sQuick Start Guide%2$s.', 'artpulse' ), '<a href="' . esc_url( $link ) . '">', '</a>' ) . ' <a href="' . esc_url( $dismiss_url ) . '">' . esc_html__( 'Dismiss', 'artpulse' ) . '</a></p>';
		echo '</div>';
	}
