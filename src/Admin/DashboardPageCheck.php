...
class DashboardPageCheck {
	...
	public static function register(): void {
		add_action( 'admin_init', array( self::class, 'check_pages' ) );
	}
	...
	public static function check_pages(): void {
		...
		if ( isset( $_GET['ap_create_dashboard_pages'] ) && check_admin_referer( 'ap_create_dashboard_pages' ) ) {
			...
			ShortcodePages::create_pages( array( '[' . $tag . ']', '[user_dashboard]' ) );
			add_action(
				'admin_notices',
				static function () {
			...
			wp_safe_redirect( remove_query_arg( array( 'ap_create_dashboard_pages', '_wpnonce' ) ) );
			exit;
		}

		$tag   = apply_filters( 'ap_dashboard_shortcode_tag', 'ap_user_dashboard' );
		$codes = array( '[' . $tag . ']', '[user_dashboard]' );
