...
class OrgDashboardAdmin {
	public static function register() {
		...
		add_action( 'admin_init', array( self::class, 'handleExports' ) );
		add_action( 'admin_menu', array( self::class, 'hideFromMembers' ), 999 );
	}
	...
	private static function exportBillingCsv(): void {
		...
		fputcsv( $output, array( 'Charge ID', 'Date', 'Amount', 'Status' ) );
		foreach ( $rows as $row ) {
			fputcsv( $output, array( $row['id'], $row['date'], $row['amount'], $row['status'] ) );
		}
		fclose( $output );
		exit;
	}
	...
	private static function exportBillingPdf(): void {
		...
		if ( class_exists( Dompdf::class ) ) {
			...
			$dompdf->setPaper( 'A4' );
			$dompdf->render();
			$dompdf->stream( 'billing-' . $org_id . '.pdf', array( 'Attachment' => true ) );
		} else {
	...
	public static function clear_cache( int $post_id, \WP_Post $post, bool $update ): void {
		...
		if ( in_array( $post->post_type, array( 'ap_profile_link', 'artpulse_artwork', 'artpulse_event' ), true ) ) {
