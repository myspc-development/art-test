...
class PendingSubmissionsPage {
	...
	public static function register() {
		...
		add_action( 'admin_post_ap_reject_submission', array( self::class, 'handleRejection' ) );
	}
	...
	public static function render() {
		$posts = get_posts(
			array(
				'post_type'   => array( 'artpulse_event', 'artpulse_artist', 'artpulse_artwork', 'artpulse_org', 'ap_artist_request' ),
				'post_status' => 'pending',
				'numberposts' => 100,
			)
		);
		...
		if ( ! $posts ) {
			...
			foreach ( $posts as $post ) {
				...
				echo '<input type="hidden" name="post_id" value="' . intval( $post->ID ) . '" />';
				WpAdminFns::submit_button( __( 'Reject', 'artpulse' ), 'secondary', 'submit', false, array( 'onclick' => 'return confirm("' . esc_js( __( 'Are you sure?', 'artpulse' ) ) . '");' ) );
				echo '</form>';
				echo '</td>';
				echo '</tr>';
			}
