...
class MetaBoxesRelationship {
	...
	private static array $relationship_boxes = array(
		array(
	...
	public static function register() {
		add_action( 'add_meta_boxes', array( self::class, 'add_relationship_meta_boxes' ) );
		add_action( 'save_post', array( self::class, 'save_relationship_meta_boxes' ), 10, 1 ); // save_post typically passes post_id, and optionally post object and update status
		add_action( 'admin_enqueue_scripts', array( self::class, 'enqueue_admin_assets' ) );
		add_action( 'wp_ajax_ap_search_posts', array( self::class, 'ajax_search_posts' ) );
	}
	...
	public static function add_relationship_meta_boxes() {
		foreach ( self::$relationship_boxes as $box ) {
			...
			add_meta_box(
				...
				array( self::class, 'render_relationship_meta_box' ),
	...
	public static function render_relationship_meta_box( $post, $callback_args ) {
		...
		if ( $box['multiple'] ) {
			if ( ! is_array( $selected_ids ) ) {
				$selected_ids = empty( $selected_ids ) ? array() : array( $selected_ids ); // Handle case where single value might be stored
			}
		} else {
		...
		} elseif ( ! empty( $selected_ids ) ) { // Single select
	...
	public static function enqueue_admin_assets( $hook ) {
		...
		wp_localize_script(
			...
			array(
				...
				'nonce'            => wp_create_nonce( 'ap_search_posts' ),
	...
	public static function ajax_search_posts() {
		...
		if ( ! wp_verify_nonce( $nonce, 'ap_search_posts' ) ) {
		...
		if ( ! in_array( $post_type_to_search, $allowed_post_types, true ) && ! post_type_exists( $post_type_to_search ) ) {
			wp_send_json_error( array( 'message' => 'Invalid post type for search.' ), 400 );
			return;
		}
		...
		wp_send_json_success( array( 'results' => $results ) ); // Select2 expects results in a 'results' key for AJAX
	}
}
