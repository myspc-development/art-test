...
class MetaBoxesEvent {
	...
	public static function register() {
		...
		add_action( 'add_meta_boxes_artpulse_event', array( self::class, 'add_event_meta_boxes' ) );
		add_action( 'save_post_artpulse_event', array( self::class, 'save_event_meta' ), 10, 2 ); // Corrected CPT slug
	...
	public static function add_event_meta_boxes() {
		add_meta_box(
			...
			array( self::class, 'render_event_details' ),
		...
		foreach ( $fields as $key => $args ) {
			...
			switch ( $type ) {
				...
				case 'number':
					...
					if ( $key === 'event_rsvp_limit' ) {
						if ( $min_lim ) {
							$extra .= ' min="' . esc_attr( $min_lim ) . '"'; }
						if ( $max_lim ) {
							$extra .= ' max="' . esc_attr( $max_lim ) . '"'; }
					}
	...
	public static function save_event_meta( $post_id, $post ) {
		if ( ! isset( $_POST['ead_event_meta_nonce_field'] ) || ! wp_verify_nonce( $_POST['ead_event_meta_nonce_field'], 'ead_event_meta_nonce' ) ) {
			return;
		}
		...
		foreach ( $fields as $field => $args ) {
			...
			if ( $type === 'textarea' ) {
				if ( in_array( $field, array( 'event_rsvp_list', 'event_waitlist' ) ) ) {
	...
	private static function get_registered_event_meta_fields() {
		...
		return array(
			'_ap_event_date'         => array(
				'type'  => 'date',
				'label' => __( 'Event Date', 'artpulse' ),
			),
			'_ap_event_location'     => array(
				'type'  => 'text',
				'label' => __( 'Location', 'artpulse' ),
			),
			'_ap_event_address'      => array(
				'type'  => 'text',
				'label' => __( 'Address', 'artpulse' ),
			),
			'_ap_event_start_time'   => array(
				'type'  => 'text',
				'label' => __( 'Start Time', 'artpulse' ),
			),
			'_ap_event_end_time'     => array(
				'type'  => 'text',
				'label' => __( 'End Time', 'artpulse' ),
			),
			'_ap_event_contact'      => array(
				'type'  => 'text',
				'label' => __( 'Contact', 'artpulse' ),
			),
			'_ap_event_rsvp'         => array(
				'type'  => 'url',
				'label' => __( 'RSVP URL', 'artpulse' ),
			),
			'event_start_date'       => array(
				'type'  => 'date',
				'label' => __( 'Start Date', 'artpulse' ),
			),
			'event_end_date'         => array(
				'type'  => 'date',
				'label' => __( 'End Date', 'artpulse' ),
			),
			'event_recurrence_rule'  => array(
				'type'  => 'text',
				'label' => __( 'Recurrence Rule', 'artpulse' ),
			),
			'venue_name'             => array(
				'type'  => 'text',
				'label' => __( 'Venue Name', 'artpulse' ),
			),
			...
			'event_organizer_name'   => array(
				'type'  => 'text',
				'label' => __( 'Organizer Name', 'artpulse' ),
			),
			'event_organizer_email'  => array(
				'type'  => 'email',
				'label' => __( 'Organizer Email', 'artpulse' ),
			),
			'event_banner_id'        => array(
				'type'  => 'media',
				'label' => __( 'Event Banner (Media Library ID)', 'artpulse' ),
			),
			'event_featured'         => array(
				'type'  => 'checkbox',
				'label' => __( 'Request Featured', 'artpulse' ),
			),
			'is_featured'            => array(
				'type'  => 'checkbox',
				'label' => __( 'Featured Listing', 'artpulse' ),
			),
			'event_rsvp_enabled'     => array(
				'type'  => 'checkbox',
				'label' => __( 'Enable RSVPs', 'artpulse' ),
			),
			'event_rsvp_limit'       => array(
				'type'  => 'number',
				'label' => __( 'RSVP Limit', 'artpulse' ),
			),
			'event_waitlist_enabled' => array(
				'type'  => 'checkbox',
				'label' => __( 'Enable Waitlist', 'artpulse' ),
			),
			'_ap_event_artists'      => array(
				'type'  => 'text',
				'label' => __( 'Co‑Host Artists', 'artpulse' ),
			),
			'event_rsvp_list'        => array(
				'type'  => 'textarea',
				'label' => __( 'RSVP List', 'artpulse' ),
			),
			'event_waitlist'         => array(
				'type'  => 'textarea',
				'label' => __( 'Waitlist', 'artpulse' ),
			),
			'event_attended'         => array(
				'type'  => 'textarea',
				'label' => __( 'Attended', 'artpulse' ),
			),
		);
	}
	...
	public static function add_admin_filters() {
		...
		if ( ! $screen || $screen->post_type !== 'artpulse_event' ) {
			return; // Corrected CPT slug
		}
	...
	public static function filter_admin_query( $query ) {
		if ( ! is_admin() || ! $query->is_main_query() || $query->get( 'post_type' ) !== 'artpulse_event' ) {
			return; // Corrected CPT slug
		}
