...
class DashboardFeedbackManager
{
    public static function register(): void
    {
        ...
        add_action('wp_ajax_ap_dashboard_feedback', [self::class, 'handle']);
    }
    ...
    public static function handle(): void
    {
        ...
        if ($message === '') {
            wp_send_json_error(['message' => __('Message required.', 'artpulse')]);
        }
        $user_id = get_current_user_id();
        $role = DashboardController::get_role($user_id);
        global $wpdb;
        $wpdb->insert($wpdb->prefix . 'dashboard_feedback', [
