...
class EventViewCounter {
	...
	public static function register(): void {
		add_action( 'wp', array( self::class, 'track' ) );
		add_action( 'ap_favorite_added', array( self::class, 'track_favorite' ), 10, 3 );
		add_action( 'ap_event_shared', array( self::class, 'track_share' ), 10, 2 );
	}
	...
	public static function track_share( int $event_id ): void {
		...
		if ( $post && in_array( $post->post_type, array( 'artpulse_event', 'artpulse_artwork' ), true ) ) {
			global $wpdb;
