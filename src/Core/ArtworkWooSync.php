...
class ArtworkWooSync
{
    ...
    {
        add_action('save_post_artpulse_artwork', [self::class, 'sync_product'], 20, 2);
        add_action('woocommerce_order_status_completed', [self::class, 'notify_purchase'], 10, 1);
        add_action('woocommerce_admin_order_data_after_billing_address', [self::class, 'tracking_fields']);
        add_action('woocommerce_process_shop_order_meta', [self::class, 'save_tracking_fields']);
    }
    ...
    public static function notify_purchase($order_id): void
    {
        ...
        foreach ($order->get_items() as $item) {
            ...
            $tracking_info= $tracking ? "\n" . sprintf(__('Tracking: %s %s', 'artpulse'), $provider, $tracking) : '';
